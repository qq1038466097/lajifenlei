...
const EditMenu = ({
	...
	return (
		<div>
			{
				data.length>0&&JSON.stringify(dataInfo)!=='{}'?
				<div>
					<Form onSubmit={handleSubmit}>
						<Card title='新增菜单'>
							<FormItem {...formItemLayout} label="菜单名称">
							{getFieldDecorator('characterization',{
								initialValue: dataInfo.characterization,
								rules: [{
									required: true,
									message: '请输入角色名称',
								}],
							...
							<FormItem {...formItemLayout} label="排序">
								{getFieldDecorator('sort',{
									initialValue: dataInfo.sort,
								})(
									<InputNumber placeholder={dataInfo.sort} value={dataInfo.sort} min={1} max={10}/>
								)}
...
export default connect(({ editMenu }) => ({ editMenu }))(Form.create()(EditMenu))
