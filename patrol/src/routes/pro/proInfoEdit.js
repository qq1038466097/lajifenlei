...
import { $,validatePhone } from '../../utils/config.js'
...
const ProInfoEdit = ({
  ...
  const isLoading = loading.effects['proInfoEdit/getUsers']
    ...
    $.ajax({
      ...
      success: function (json) {
          if(json.status == 0) {
              ...
              dispatch({
                type: 'proInfoEdit/setLat',
                lat: _lat,
                lng: _lng,
              })

              dispatch({
                type: 'proInfoEdit/isSetMap',
                payload: true
              })
          } else {
...
export default connect(({ proInfoEdit,loading }) => ({ proInfoEdit,loading }))(Form.create()( ProInfoEdit ))
